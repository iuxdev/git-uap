! function() {
	...
	var e = {
		...
		module: function() {
			this.queryLoader(), this.sticky(), this.zoomImage(), this.viewport(), this.flickr(), this.menu(), this.knob(), this.fitvids(), this.progressBar(), this.tubular(), this.datePicker(), this.roomSelect(), this.parallaxEff(), this.customScroll()
		},
		...
		fitvids: function() {
			...
			1 === e.trim().search("iframe") && jQuery("noscript").parent().append(e), jQuery("#home").fitVids({
		...
		twitter: function() {
			...
			jQuery(".twitter-widget").each(function() {
				...
				$.getJSON("php/twitter423c.html?user=" + n, function(t) {
					r.empty();
					for (var i = 0; s > i && i < t.length; i++) {
						var n, u = Math.floor((o.getTime() - Date.parse(t[i].created_at)) / 1e3),
							a = u % 60;
						u = Math.floor(u / 60);
						var c = u % 60;
						if (c) {
		...
		datePicker: function() {
			jQuery("#check-in").datetimepicker({
				...
				onShow: function() {
					this.setOptions({
						maxDate: jQuery("#check-out").val() ? jQuery("#check-out").val() : !1
					})
				},
				timepicker: !1
				...
				onShow: function() {
					this.setOptions({
						minDate: jQuery("#check-in").val() ? jQuery("#check-in").val() : !1
					})
				},
				timepicker: !1
			})
